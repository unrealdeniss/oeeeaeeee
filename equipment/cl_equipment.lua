...
File Path:   addons/zeros_methlab02_v1.4.8/lua/zmlab2/equipment/cl_equipment.lua
...
net.Receive("zmlab2_Equipment_Repair_UI", function(len)
    zclib.Debug_Net("zmlab2_Equipment_Repair_UI", len)
    local Equipment = net.ReadEntity()
    if not IsValid(Equipment) then return end
...
function zmlab2.Equipment.Repair(Equipment)
    zclib.PointerSystem.Start(Equipment,function()
        ...
        net.Start("zmlab2_Equipment_Repair_Request")
        net.WriteEntity(zclib.PointerSystem.Data.Target)
        net.SendToServer()
    end,function()
        ...
        if IsValid(zclib.PointerSystem.Data.Target) then
            ...
            local cls    = zclib.PointerSystem.Data.Target:GetClass()
            local max    = zmlab2.config.Damageable[cls] or 0
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 76561198010069529
            ...
            if max > 0 then
                ...
                draw.SimpleText("[" .. val .. "/" .. max .. "]",
                    zclib.GetFont("zclib_font_big"),
                    data2D.x, data2D.y,
                    color_white,
